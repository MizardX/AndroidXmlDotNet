...
namespace AndroidXml
{
    public class AndroidXmlReader : XmlReader
    {
        ...
        public ResStringPool StringPool => _parser.Strings;

        private void AddNamespace(string prefix, string uri)
        {
            if (!_namespaces.TryGetValue(prefix, out var definitions))
            {
        ...
        private string FormatValue(Res_value value)
        {
            ...
            switch (value.DataType)
            {
                ...
                case Res.ValueType.TYPE_FRACTION:
                    ...
                    unit = index0 < 2 ? new[] { "%", "%p" }[index0] : "?";
                    return $"{value.ComplexValue:g}{unit}";
                case Res.ValueType.TYPE_DIMENSION:
                    index0 = (int)value.ComplexDimensionUnit;
                    unit = index0 < 6 ? new[] { "px", "dip", "sp", "pt", "in", "mm" }[index0] : "?";
                    return $"{value.ComplexValue:g}{unit}";
        ...
        {
            _namespaces.TryGetValue(prefix, out var definitions);
        ...
        /// </returns>
        public override int AttributeCount => (int)_parser.AttributeCount;
